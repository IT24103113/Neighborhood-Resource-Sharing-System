...
function RequestCard({ request, onPress }) {
  ...
  const lenderName = request.lender_id?.name || "Unknown lender";

  return (
    <Pressable style={styles.card} onPress={onPress}>
      <Text style={styles.cardTitle}>{itemTitle}</Text>
      <Text style={styles.cardMeta}>Lender: {lenderName}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>{isLoading ? "Loading requests..." : "No sent requests yet."}</Text>
        }
